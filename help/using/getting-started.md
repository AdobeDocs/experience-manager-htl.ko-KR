...
* **블록 문** - 조건부로 `<h1>` 요소를 표시하려면 `data-sly-test` HTML5 데이터 특성을 사용하십시오. HTL은 이러한 여러 속성을 제공하여 모든 HTML 요소에 동작을 첨부할 수 있으며 모두 `data-sly` 접두사가 붙습니다.
* **표현식 언어** - `${` 및 `}`자는 HTL 표현식을 구분합니다. 런타임 시 이러한 표현식이 평가되며 해당 값은 발신 HTML 스트림에 삽입됩니다.

두 구문에 대한 자세한 내용은 [HTL 사양](specification.md)을 참조하십시오.

### SLY 요소 {#the-sly-element}

HTL의 핵심 개념은 블록 문을 정의하기 위해 기존 HTML 요소를 재사용할 수 있는 가능성을 제공하는 것입니다. 이렇게 다시 사용하면 명령문이 시작하고 끝나는 위치를 정의하기 위해 추가 구분 기호를 삽입할 필요가 없습니다. 마크업에 주석을 다는 것은 정적 HTML을 HTML의 유효성을 손상시키지 않고 동적 템플릿으로 비간섭적으로 변환하여 정적 파일로도 적절히 표시할 수 있도록 합니다.

단, 블록 문이 삽입되어야 하는 정확한 위치에 기존 요소가 없는 경우도 있을 수 있습니다. 이러한 경우 특수 `sly` 요소를 삽입할 수 있습니다. 이 요소는 첨부된 블록 문을 실행하고 그에 따라 콘텐츠를 표시하는 동안 출력에서 자동으로 제거됩니다.
...
유념해야 할 한 가지는 기존 요소에 블록 문으로 주석을 추가할 수 없는 경우에만 `sly` 요소를 사용하는 것입니다. 이유는 `sly` 요소가 언어에서 제공하는 값이 정적 HTML을 동적으로 만들 때 변경하지 않도록 하기 때문입니다.
...
그리고 `div` 요소에 다음 조건으로 주석을 추가할 수 있습니다.
...
HTL 댓글은 JavaScript와 유사한 추가 구문이 있는 HTML 주석입니다. 프로세서는 전체 HTL 댓글과 그 안의 모든 항목을 무시하여 출력에서 제거합니다.
...
표현식은 HTML 텍스트 또는 속성 값에만 배치할 수 있지만 요소 이름이나 속성 이름에는 배치할 수 없습니다. 그렇지 않으면 더 이상 유효한 HTML이 아닙니다. 요소 이름을 동적으로 설정하려면 원하는 요소에 `data-sly-element` 문을 사용할 수 있으며, 특성 이름을 동적으로 설정하려면 여러 특성을 한 번에 설정하는 경우에도 `data-sly-attribute` 문을 사용할 수 있습니다.
...
HTL은 블록 문을 정의하기 위해 데이터 속성을 사용하므로 다음 컨텍스트 내에서 이러한 블록 문을 정의할 수 없으며 표현식만 사용할 수 있습니다.

* HTML 주석
* 스크립트 요소
* 스타일 요소

그 이유는 이러한 컨텍스트의 콘텐츠가 HTML이 아닌 텍스트이고 포함된 HTML 요소가 단순 문자 데이터로 간주되기 때문입니다. 따라서 실제 HTML 요소가 없으면 `data-sly` 특성도 실행할 수 없습니다.

이러한 접근은 상당한 제한처럼 들릴 수 있다. 그러나 HTML 템플릿 언어는 유효한 HTML 출력만 생성해야 하므로 이 언어가 선호됩니다. 아래 [액세스 논리를 위한 Use-API](#use-api-for-accessing-logic) 섹션에서는 템플릿에서 추가 논리를 호출하는 방법을 소개합니다. 이러한 컨텍스트에 대한 복잡한 출력을 준비하는 데 필요한 경우 이 방법을 사용할 수 있습니다. 백 엔드에서 프론트엔드 스크립트로 데이터를 보내려면 구성 요소의 논리로 JSON 문자열을 생성하고 간단한 HTL 표현식을 사용하여 데이터 속성에 배치합니다.
...
아래 [자동 컨텍스트 인식 이스케이프](#automatic-context-aware-escaping) 섹션에 설명된 대로 HTL의 한 가지 목표는 모든 표현식에 컨텍스트 인식 이스케이프를 자동으로 적용하여 XSS(크로스 사이트 스크립팅) 취약성 유발의 위험을 줄이는 것입니다. HTL은 HTML 마크업에서 표현식의 컨텍스트를 감지하지만 인라인 JavaScript 또는 CSS는 분석하지 않으므로 개발자는 이러한 표현식에 대한 정확한 컨텍스트를 지정해야 합니다.
...
HTML 템플릿 언어(HTL) Java Use-API를 사용하면 HTL 파일이 `data-sly-use`를 통해 사용자 지정 Java 클래스의 도우미 메서드에 액세스할 수 있습니다. 이 프로세스를 사용하면 모든 복잡한 비즈니스 논리를 Java 코드에 캡슐화할 수 있지만 HTL 코드는 직접 마크업 생성만 처리합니다.
...
이를 위해 HTML 템플릿 언어는 변수가 배치된 컨텍스트에 따라 각 변수를 자동으로 이스케이프합니다. HTL이 HTML 구문을 이해한다는 사실 덕분에 이 접근 방식이 가능합니다.
...
`class` 속성의 값이 비어 있으면 HTML 템플릿 언어는 자동으로 전체 `class` 특성을 출력에서 제거합니다.

다시 말하지만, HTL이 HTML 구문을 이해하고 따라서 값이 비어 있지 않은 경우에만 동적 값이 있는 속성을 조건부로 표시할 수 있기 때문에 이 프로세스가 가능합니다. 마크업을 유효하지 않고 읽을 수 없게 만들 수 있는 속성에 조건 블록을 추가하는 것을 방지하기 때문에 매우 편리합니다.

또한 표현식에 배치된 변수의 유형이 중요합니다.

* **String:**
   * **not empty:** 문자열을 특성 값으로 설정합니다.
   * **empty:** 속성을 모두 제거합니다.
...
이 섹션에서는 몇 가지 일반적인 시나리오를 소개합니다. HTML 템플릿 언어로 이러한 시나리오를 해결하는 가장 좋은 방법을 설명합니다.
...
이 예제에서 HTML `head`과(와) `body` 요소가 별도의 파일에 있는 경우 `clientlib.html` 템플릿은 필요한 각 파일에 로드되어야 합니다.
...
다음 예제에서는 클라이언트에 전달하기 위해 개체를 JSON(Java로도 가능)으로 직렬화하는 방법을 보여 줍니다. 그런 다음 `data` 특성에 쉽게 배치할 수 있습니다.
...
거기에서 클라이언트측 JavaScript가 해당 속성에 액세스하고 JSON을 다시 구문 분석하는 방법을 쉽게 상상할 수 있습니다. 이 접근 방식은 클라이언트 라이브러리에 배치할 해당 JavaScript입니다(예: ).
...
Handlebars 템플릿 콘텐츠가 자체 파일에 격리되어 있으므로 `script` 요소의 태그에는 명시적 컨텍스트 없이 HTL 블록 문이 포함될 수 있습니다. 또한 이 예에서는 서버측 실행 HTL(`h2` 요소에서와 같이)이 핸들바(`h3` 요소에 표시된)와 같은 클라이언트측 실행 템플릿 언어와 어떻게 혼합될 수 있는지 보여 줍니다.
...
스크립트, 스타일 및 주석 컨텍스트의 제한을 피해야 하는 특별한 경우에는 해당 콘텐츠를 별도의 HTL 파일로 격리할 수 있습니다. HTL은 자체 파일의 모든 항목을 표준 HTML 조각으로 해석하여 포함된 위치에서 제한된 컨텍스트를 무시합니다.

예를 보려면 더 아래에 있는 [클라이언트측 템플릿 작업](#working-with-client-side-templates) 섹션을 참조하십시오.

>[!CAUTION]
>
>이 기술은 XSS(크로스 사이트 스크립팅) 취약성을 유발할 수 있습니다. 이러한 접근 방식을 사용할 경우 보안 측면을 주의 깊게 검토해야 합니다. 일반적으로 이 방법에 의존하는 것보다 동일한 것을 구현하는 더 좋은 방법이 있습니다.
