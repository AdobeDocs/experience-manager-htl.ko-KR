...
HTML 템플릿 언어(HTL) Java Use-API를 사용하면 HTL 파일이 `data-sly-use`을 통해 사용자 지정 Java 클래스의 헬퍼 메서드에 액세스할 수 있습니다. 이렇게 하면 모든 복잡한 비즈니스 로직을 Java 코드로 캡슐화할 수 있고, HTL 코드는 직접 마크업 프로덕션만 다룹니다.
...
`bindings` 맵에는 Use-API 개체가 해당 처리에 사용할 수 있는 현재 실행되는 HTL 스크립트에 컨텍스트를 제공하는 개체가 포함될 수 있습니다.

## 간단한 예 {#a-simple-example}

사용 클래스가 없는 HTL 구성 요소로 시작합니다. 단일 파일 `/apps/my-example/components/info.html`으로 구성됩니다.
...
또한 이 구성 요소에 대해 `/content/my-example/`에서 렌더링할 컨텐트도 추가되었습니다.
...
이 콘텐트에 액세스하면 HTL 파일이 실행됩니다. HTL 코드 내에서 컨텍스트 개체 `properties`을 사용하여 현재 리소스의 `title` 및 `description`에 액세스하여 표시합니다. 출력 HTML은 다음과 같습니다.
...
The **info** component as it standards not need a use-class to perform its (very simple) function. 그러나 HTL에서는 불가능한 일을 해야 하므로 사용 클래스가 필요합니다. 그러나 다음 사항에 주의하십시오.

>[!NOTE]
>
>use-class는 HTL에서만 수행할 수 없는 경우에만 사용해야 합니다.

예를 들어 `info` 구성 요소가 리소스의 `title` 및 `description` 속성을 표시하지만 모두 소문자로만 표시한다고 가정합니다. HTL에는 문자열 소문자에 대한 메서드가 없으므로 use-class가 필요합니다. 이렇게 하려면 Java 사용 클래스를 추가하고 `info.html`을 다음과 같이 변경하면 됩니다.
...
>
>Java 코드가 여러 HTL 구성 요소에서 액세스하는 서비스를 구현할 경우 **번들 Java use-class**&#x200B;를 권장합니다.
...
>[!NOTE]
>
>저장소 항목 이름에 하이픈을 사용하는 것이 AEM 개발에서 권장됩니다. 그러나 Java 패키지 이름 내에서는 하이픈이 잘못되었습니다. 따라서 저장소 경로에 있는 **모든 하이픈을 패키지 이름**&#x200B;에 있는 밑줄로 변환해야 합니다.
...
### 클래스 {#initializing-the-class}을(를) 초기화하는 중
...
일반적으로 [activate](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/sightly/WCMUsePojo.html) 메서드는 현재 컨텍스트(예: 현재 요청 및 리소스)를 기반으로 HTL 코드에 필요한 값을 미리 계산하여 저장하는 데 사용됩니다.

`WCMUsePojo` 클래스는 HTL 파일 내에서 사용할 수 있는 동일한 컨텍스트 개체 집합에 대한 액세스를 제공합니다([전역 개체](global-objects.md) 참조).

`WCMUsePojo`을(를) 확장하는 클래스에서는
...
또는 일반적으로 사용되는 컨텍스트 개체는 적절한 **편의 메서드**&#x200B;에서 직접 액세스할 수 있습니다.
...
* `getXyz` 형식의 메서드는 HTL 파일 내에 `xyz`이라는 개체 속성이 노출됩니다.
...
`data-sly-use` 속성은 HTL 코드 내에서 use-class를 초기화하는 데 사용됩니다. 예제에서 `data-sly-use` 속성은 `Info` 클래스를 사용한다고 선언합니다. 로컬 설치를 사용하고 있으므로 클래스의 로컬 이름만 사용할 수 있습니다. Java 소스 파일은 HTL 파일과 동일한 폴더에 있습니다. 번들 설치를 사용하는 경우 정규화된 클래스 이름을 지정해야 합니다.
...
식별자 `info`(`data-sly-use.info`의 점 뒤)는 HTL 파일 내에서 클래스를 식별하는 데 사용됩니다. 이 식별자의 범위는 선언된 후 파일 내에서 전역적입니다. 이 값은 `data-sly-use` 문을 포함하는 요소에 제한되지 않습니다.
...
여기서 `text`이라는 매개 변수를 전달합니다. 그런 다음 use-class는 검색하고 결과를 `info.upperCaseText`으로 표시합니다. 다음은 조정된 사용 클래스입니다.
...
이유는 use-class가 항상 HTL 코드와 동일한 실행 컨텍스트에 액세스할 수 있기 때문입니다. 이를 통해 최상의 작업 방법을 가져올 수 있습니다.

>[!NOTE]
>
>use-class에 매개 변수를 전달하는 것은 전달해야 하는 매개 변수가 있는 다른 HTL 파일에서 호출되는 use-class를 `data-sly-template` 파일에서 사용하는 경우에만 수행해야 합니다.
...
`extra` 템플릿은 단일 매개 변수 `text`를 사용합니다. 그런 다음 Java use-class `ExtraHelper`을 로컬 이름 `extraHelper`으로 초기화하고 템플릿 매개 변수 `text`의 값을 use-class 매개 변수 `text`으로 전달합니다.

템플릿의 본문은 속성 `extraHelper.reversedText`을(를) 가져오고(백그라운드에서 실제로 `ExtraHelper.getReversedText()` 호출) 값을 표시합니다.

또한 이 새 템플릿을 사용하도록 기존 `info.html`도 조정했습니다.
...
이제 `info.html` 파일에는 두 개의 `data-sly-use` 문이 포함되어 있습니다. 이 문은 `Info` Java use-class를 가져오는 원본 문과 로컬 이름 `extra`의 템플릿 파일을 가져오는 새 명령문입니다.

두 번째 `data-sly-use`을(를) 피하기 위해 `info.html` 파일 내에 템플릿 블록을 배치할 수 있었지만 별도의 템플릿 파일은 더 일반적이며 더 재사용 가능합니다.

`Info` 클래스는 이전과 같이 사용되어 해당 HTL 속성 `info.lowerCaseTitle` 및 `info.lowerCaseDescription`을 통해 getter 메서드 `getLowerCaseTitle()` 및 `getLowerCaseDescription()`를 호출합니다.

그런 다음 템플릿 `extra`에 `data-sly-call`을 수행하고 `properties.description` 값을 매개 변수 `text`으로 전달합니다.

Java use-class `Info.java`이 새 텍스트 매개 변수를 처리하도록 변경되었습니다.
...
`text` 매개 변수는 `get("text", String.class)`과(와) 함께 검색되고, 값이 반전되어 getter `getReversedText()`을 통해 HTL 개체 `reversedText`로 사용할 수 있게 됩니다.

### 번들 Java 클래스 {#bundled-java-class}

번들 사용 클래스를 사용하면 표준 OSGi 번들 배포 메커니즘을 사용하여 클래스를 컴파일하고, 패키지하여 AEM에 배포해야 합니다. 로컬 설치와는 대조적으로 use-class **package 선언**&#x200B;의 이름은 정상적으로 지정해야 합니다.
...
Java 사용 클래스를 만드는 가장 일반적인 방법은 `WCMUsePojo`을 확장하는 것입니다. 그러나 다른 옵션들은 많이 있습니다. 이러한 변형을 이해하기 위해 HTL `data-sly-use` 문이 백그라운드에서 작동하는 방식을 이해하는 데 도움이 됩니다.
...
* HTL 파일과 동일한 디렉토리에 로컬 파일 `UseClass.java`이 있는 경우 해당 클래스를 컴파일하고 로드해 봅니다. 성공한 경우 (2)로 이동합니다.
* 그렇지 않으면 `UseClass`을(를) 정규화된 클래스 이름으로 해석하고 OSGi 환경에서 로드해 보십시오. 성공한 경우 (2)로 이동합니다.
* 그렇지 않으면 `UseClass`을 HTL 또는 JavaScript 파일의 경로로 해석하고 해당 파일을 로드합니다. 성공적으로 goto (4)를 선택한 경우

(2)

* 현재 `Resource`을(를) `UseClass`에 적용하려고 합니다. 성공하면 (3)으로 이동합니다.
* 그렇지 않은 경우 현재 `Request`을 `UseClass`로 조정하십시오. 성공하면 (3)으로 이동합니다.
* 그렇지 않은 경우 인수 0인 생성자로 `UseClass`을(를) 인스턴스화하십시오. 성공하면 (3)으로 이동합니다.

(3)

* HTL 내에서 새로 채택되었거나 생성된 개체를 이름 `localName`으로 바인딩합니다.
* `UseClass`이 [`io.sightly.java.api.Use`](https://helpx.adobe.com/experience-manager/6-2/sites/developing/using/reference-materials/javadoc/io/sightly/java/api/Use.html)를 구현한 경우 현재 실행 컨텍스트를 전달하여 `init` 메서드를 호출합니다.`javax.scripting.Bindings`

(4)

* `UseClass`이(가) `data-sly-template`이 포함된 HTML 파일의 경로인 경우 템플릿을 준비하십시오.
* 그렇지 않으면 `UseClass`이(가) JavaScript use-class의 경로인 경우 use-class를 준비하십시오([JavaScript Use-API](use-api-javascript.md) 참조).

위의 설명에 대한 몇 가지 중요 사항:

* `Resource`에서 적응할 수 있는 클래스, `Request`에서 적응할 수 있는 클래스 또는 인수가 0인 생성자가 있는 모든 클래스는 use-class일 수 있습니다. 클래스는 `WCMUsePojo`을(를) 확장하거나 `Use`을(를) 구현할 필요가 없습니다.
* 그러나 use-class *가*&#x200B;을(를) 구현할 경우 해당 `init` 메서드는 현재 컨텍스트와 함께 자동으로 호출되므로 해당 컨텍스트에 의존하는 초기화 코드를 여기에 배치할 수 있습니다.`Use`
* `WCMUsePojo`을(를) 확장하는 use-class는 `Use`을(를) 구현하는 특수한 경우일 뿐입니다. 편리한 컨텍스트 메서드를 제공하고 해당 `activate` 메서드는 `Use.init`에서 자동으로 호출됩니다.

### 인터페이스 직접 구현 {#directly-implement-interface-use}
...
`init` 메서드는 모든 컨텍스트 개체와 use-class로 전달된 모든 매개 변수를 포함하는 `Bindings` 개체가 있는 클래스의 초기화에 대해 호출됩니다.

모든 추가 기능(예: `WCMUsePojo.getProperties()`에 상응하는)은 [`javax.script.Bindings`](http://docs.oracle.com/javase/7/docs/api/javax/script/Bindings.html) 개체를 사용하여 명시적으로 구현해야 합니다. 예:
...
`WCMUsePojo`을(를) 확장하지 않고 직접 `Use` 인터페이스를 구현하기 위한 기본 사례는 기존 클래스의 하위 클래스를 use-class로 사용하려는 경우입니다.

### 리소스 {#adaptable-from-resource}에서 적응할 수 있습니다.

다른 옵션은 `org.apache.sling.api.resource.Resource`에서 사용할 수 있는 도우미 클래스를 사용하는 것입니다.

DAM 자산의 MIMETYPE을 표시하는 HTL 스크립트를 작성해야 한다고 가정해 보겠습니다. 이 경우 HTL 스크립트가 호출되면 JCR `Node`을 nodetype `dam:Asset`으로 래핑하는 `Resource` 컨텍스트 내에 있게 됩니다.
...
여기서는 예제 프로젝트 geometrixx의 일부로 AEM의 기본 설치와 함께 제공되는 에셋(JPEG 이미지)을 보여줍니다. 자산의 이름은 `jane_doe.jpg`이고 해당 mimetype은 `image/jpeg`입니다.
...
`data-sly-use` 문은 HTL이 현재 `Resource`을 `Asset`로 적용하고 로컬 이름 `asset`을 지정합니다. 그런 다음 HTL getter 축약어를 사용하여 `Asset`의 `getMimeType` 메서드를 호출합니다.`asset.mimeType`.

### 요청 {#adaptable-from-request}에서 적응할 수 있습니다.

또한 [`org.apache.sling.api.SlingHttpServletRequest`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/SlingHttpServletRequest.html)에서 사용할 수 있는 모든 클래스를 use-class로 사용할 수도 있습니다

`Resource`에서 사용할 수 있는 use-class 어댑터의 상기와 마찬가지로 [`SlingHttpServletRequest`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/SlingHttpServletRequest.html)의 use-class 어댑터를 `data-sly-use` 문에서 지정할 수 있습니다. 실행 시 현재 요청은 주어진 클래스에 맞게 조정되며, 결과 개체는 HTL 내에서 사용할 수 있게 됩니다.
