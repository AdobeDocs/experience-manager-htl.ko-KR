...
`/content/my-example/`에서 렌더링할 이 구성 요소에 대한 몇 가지 콘텐츠도 추가합니다.
...
Java 클래스를 HTL과 통합하는 방법은 여러 가지가 있지만([`WCMUsePojo`](#alternatives-to-wcmusepojo)에 대한 대안 섹션 참조) 가장 간단한 방법은 `WCMUsePojo` 클래스를 확장하는 것입니다. 이 `/apps/my-example/component/info/Info.java` 예제의 경우:
